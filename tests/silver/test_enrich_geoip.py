...
####----                                                    ----####
####----  Formato Parquet em vez de JSON: mais compacto     ----####
####----  (colunar + compressão) e já é o formato usado no  ----####
####----  restante do pipeline (Bronze/Silver/Gold).        ----####
####------------------------------------------------------------####
...
def load_geoip_cache_from_r2(
    ...
    try:
        ...
        print(f"Cache GeoIP não encontrado em r2://{bucket}/{key}. Iniciando vazio.")
        return {}
    except Exception as exc:
        # Cobre ClientError genérico (ex.: 404 sem NoSuchKey tipado em
        # alguns clients) sem derrubar o pipeline por causa do cache.
        print(f"Falha ao carregar cache GeoIP do R2 ({exc}). Iniciando vazio.")
        return {}
...
####---------------------------------------------------------------------------------------####
####----                                                                               ----####
####----  Desenho (decisão registrada na conversa com a Carla):                        ----####
...
####----                                                                               ----####
####----  Severidade da divergência (tabela definida pela Carla):                      ----####
####----    ≤100 km                      → baixa                                       ----####
####----    100–500 km                    → média                                      ----####
